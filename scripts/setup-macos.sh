...
echo ""
echo -e "${CYAN}==========================================${NC}"
echo -e "${CYAN}  FlowBridge - macOS Auto Setup${NC}"
echo -e "${CYAN}==========================================${NC}"
echo ""
...
echo -e "${YELLOW}[2/7] Checking Docker Desktop running...${NC}"
if ! docker ps &> /dev/null; then
    echo -e "${RED}  ERROR: Docker Desktop not running. Start it and rerun.${NC}"
    exit 1
fi
echo -e "${GREEN}  OK: Docker Desktop running${NC}"
...
echo "     - Open Finder -> Go -> Connect to Server -> vnc://localhost:5900"
echo "     - Or use RealVNC Viewer: localhost:5900 (no password)"
